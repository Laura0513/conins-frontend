...
type EditarCompetenciaModalProps = {
  isOpen: boolean
  onClose: () => void
...
export default function EditarCompetenciaModal({
  isOpen,
  onClose,
  competencia,
  onSubmit,
  programas,
}: EditarCompetenciaModalProps) {
  ...
  useEffect(() => {
    if (competencia) {
      setCodigo(competencia.codigo)
      setNombre(competencia.nombre)
      setProgramaId(competencia.programa_id)
    }
  }, [competencia])
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden">
        <div className="flex items-center justify-between p-6 border-b border-gray-100">
          <h2 className="text-lg font-bold text-gray-900">Editar competencia</h2>
